...
struct vertice {          //Estrutura Vértice (Listas Encadeadas)
...
struct grafo* construtorGrafo(int tamanho) {    //Método construtor do grafo 
...
bool verticeExiste(struct grafo *grafo, int valor) {    //Verifica se o Vértice existe no grafo
...
struct grafo* construtorVertice(struct grafo *grafo, int valor,int tipo_insercao) { //Constrói Vértice no Grafo

  if (verticeExiste(grafo, valor) == false) {      //Se o Vértice não existe no Grafo é criado um novo Vértice com esse valor
  ...
  else{  //Se o Vértice existe no Grafo a função é chamada recursivamente até encontrar um novo valor
    
    if(tipo_insercao == 0){     //Para o 1° Grafo
      
      valor = (rand() % n_vertices_1); // 0 até 9
      return construtorVertice(grafo,valor,0);
  
    }
    
    if(tipo_insercao == 1){   //Para o 2° Grafo
      
      valor = (rand() % n_vertices_2); // 0 até 999
...
int n_arestaAdjacentes(struct grafo *grafo,int valor){    //Função retorna o número de Arestas Adjacentes de um Vértice

  for (int i = 0; i <= grafo->n_vertice; i++) {
    if (grafo->vertice[i].valor == valor){
      
      int numero = 0;
      struct vertice *aux = &grafo->vertice[i];  //Criado Vértice auxiliar
...
struct grafo* construirAresta(struct grafo *grafo, int vertice1, int vertice2) { //Função que cria nova aresta não direcional 

  if(vertice1 != vertice2){  /* Motivo pelo qual os valores não podem ser iguais:

                                Se um Vértice qualquer no Grafo, por exemplo: Grafo[0] = 3 (1° Vértice na lista de vertices no grafo), e
                                Grafo[0] = 3 -> 5 -> 7, ou seja, possui arestas no vértice 5 e 7.
                                Podemos observar que o vértice 3 possui 2 arestas adjacentes, quando ocorrer a construção de uma nova aresta,
                                se essa nova aresta criada apontar para ele próprio, isto é, se a aresta for um laço, vai ocorrer o seguinte problema:
                                Grafo[0] = 3 -> 5 -> 7 -> 3 -> 3
                                O vértice 3 possui 4 arestas adjacentes, isto ocorre por conta da verificação do número de arestas ser feita antes 
                                da criação das arestas, no caso, como é uma aresta não direcional, vai ser criado um número de arestas < 4.         
                            */
    
    if(verticeExiste(grafo,grafo->vertice[vertice1].valor) == true && verticeExiste(grafo,grafo->vertice[vertice2].valor) == true){ //Se os 2 vértices existem no grafo
      
      if((n_arestaAdjacentes(grafo,grafo->vertice[vertice1].valor) < 3) && (n_arestaAdjacentes(grafo,grafo->vertice[vertice2].valor) < 3)){ //Verifica se o número de arestas é menor que 3
          ...
          if(aux->valor == novo->valor){ //Não pode ocorrer repetição de arestas adjacentes em sequência 
            free(novo);
            return grafo;
          }
          ...
          if(aux2->valor == novo2->valor){ //Não pode ocorrer repetição de arestas adjacentes em sequência 
            free(novo);
            free(novo2);
            return grafo;
          }

          //Aresta não direcional: Para a criação da aresta os 2 Vértices tem que receber na Lista de adjacência o valor do outro
          aux->next = novo;      
          aux2->next = novo2;
...
void InicializandoMatrix_1(int matriz[][TAM_1]){  //Inicializa a Matriz de Adjacência com valor -1
...
void InicializandoMatrix_2(int matriz_1[][TAM_1],int matriz_2[][TAM_2]){  //Inicializa a Matriz de Adjacência com valor -1
...
void ConstrutorVerticeMatrix_1(int matriz[][TAM_1],int valor){   //Constrói Vértice
  
  while(matriz[valor][valor] == 0 || matriz[valor][valor] == 1){ //Se o Vértice já existe, fica presa no loop até encontrar um novo valor que não seja um Vértice já criado
...
void ConstrutorVerticeMatrix_2(int matriz[][TAM_2],int valor){ //Constrói Vértice
...
int N_aresta_AdjacentesMatrix_1(int matriz[][TAM_1],int valor){ //Retorna o número de Arestas Adjacentes no vértice
...
int N_aresta_AdjacentesMatrix_2(int matriz[][TAM_2],int valor){ //Retorna o número de Arestas Adjacentes no vértice
...
bool verticeExisteMatrix_1(int matriz[][TAM_1],int valor){  //Verifica se o Vértice existe na Matriz de Adjacência
...
bool verticeExisteMatrix_2(int matriz[][TAM_2],int valor){  //Verifica se o Vértice existe na Matriz de Adjacência
...
void ConstruirArestaMatrix_1(int matriz[][TAM_1],int vertice1,int vertice2){   //Função cria nova aresta não direcional 
...
void ConstruirArestaMatrix_2(int matriz[][TAM_2],int vertice1,int vertice2){  //Função cria nova aresta não direcional 
...
int main(){
  ...
  for(int i = 0; i< n_vertices_1; i++){ //n_vertices_1 = 100
    ...
    for(int j = 0; j< numero_aleatorio;j++){
      ...
      ConstruirArestaMatrix_1(grafoMatriz,grafo_lista->vertice[i].valor,grafo_lista->vertice[numero_aleatorio2].valor); //A construção das Arestas será a mesma do grafo de Listas Adjacentes
    }
  }
  ...
  //--------------Mudando o tamanho do grafo para 1.000 vértices-----------------------
  ...
  while(laco < n_insercao){
    for(int i = comeco; i< limite; i++){  //De 10 em 10 pela Lista
  ...
  while(laco < n_insercao){
    for(int i = comeco; i< limite; i++){ //De 10 em 10 pela Tabela
      numero_aleatorio = (rand() % n_vertices_2); // 0 até 999
  ...
  //-----------Tempo para inserção das Arestas de tabela de adjacencias----------------------
